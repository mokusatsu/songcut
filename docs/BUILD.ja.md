...
自動ビルドへ `dist` の更新を明示的に依頼した場合、実行ファイルのパスが
`dist\songcut-win-x64` 内にあるプロセスは、追加の終了確認を表示せず終了して構いません。
終了前に実行ファイルのパスを確認し、別の配布物やディレクトリで動く同名プロセスは
終了しないでください。自動更新を依頼する前に、パッケージ版アプリの作業内容を保存して
ください。
...
Full版の同梱Smallは配布フォルダから読み取り専用で使用します。追加取得したTiny/Base/Small、
Hugging Faceキャッシュ、OpenVINOキャッシュは `%LOCALAPPDATA%\songcut` に保存され、
展開したポータブル配布フォルダへは書き込みません。

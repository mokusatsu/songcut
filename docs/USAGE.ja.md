...
音響検出では局所境界補正が既定で有効になり、各歌唱区間の開始・終了を音量変化に合わせて
自動調整します。有効／無効や調整内容は **Settings > 局所境界補正** で変更できます。
補正結果を詳しく確認したい場合は、対象区間を選び **Segment > 境界補正の詳細...** を開いてください。
...
微調整後は変更した境界付近が自動再生され、その場で結果を確認できます。
...
動画は既定でスマートレンダリングされます。再利用できる映像部分はそのままコピーし、区間の
境界だけを再エンコードすることで、画質を保ちながら書き出し時間を短縮します。**スマート
レンダリング詳細を確認**を押すと、クリップごとの予定を確認できます。対応しない形式や
コピー可能な部分がないクリップは、自動的に全体再エンコードへ切り替わります。

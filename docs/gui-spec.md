...
- Confirming export opens a native output directory dialog, then starts the
  backend export job.
- Export Review initially estimates smart-render compatibility from the source
  container and video codec without scanning keyframes. Each selected range is
  labeled `Not checked` until the user explicitly presses
  `Check smart rendering details`. That action evaluates the ranges and changes
  each label to green `Smart render` or amber `Full re-encode`, with the reason.
  Because eligibility depends on the range, a single export can contain both
  modes. Detailed checking is optional; export performs its own render planning.
  The displayed filename extension follows the estimated or checked output
  container.
- Detailed checking runs one selected range at a time. Its button reports
  `Checking... n/total`; completed rows immediately switch from `Not checked`
  to their result while the remaining rows stay unchanged until processed.
